...
def start(args):
    ...
    try:
        ...
        cnf_set = input_reader.get_cnf_set()

        # Tasks: Factorization
        if args.factorize:
            fact = Factorizer()
            if not fact.preprocess_set(cnf_set):
                args.factorize = False

        if args.multiply:
            mul = Multiply()
            if not mul.preprocess_set(cnf_set, args.multiply[0], args.multiply[1]):
                return

            # check if any clause evaluated to False afer substitution 
            for cl in cnf_set.clauses:
                if not cl.value:
                    print("The input set is NOT satisfiable with input factors.")
                    print(
                        f"The input numbers {args.multiply[0]} and {args.multiply[1]} can't be multiplied on the input CNF")
                    return

        # copy the cnf to be used in verification step if needed as the CNF will be subject to rename and manipulation later.
        original_cnf = deepcopy(cnf_set)
        # start processing the root set
        if len(cnf_set.clauses) > 0 or cnf_set.value is not None:
            pattern_solver = PatternSolver(args=args, problem_id=cnf_set.get_hash().hex())
            pattern_solver.solve_set(cnf_set)

            # save solution in a file
            if args.output_solution_file and pattern_solver.solution:
                solution = pattern_solver.format_solution()
                file_name = pattern_solver.problem_id
            ...
            # verify the solution
            if args.verify and pattern_solver.solution:
                if pattern_solver.verify_solution(original_cnf):
...
if __name__ == "__main__":
    ...
    group1.add_argument("-qn", "--quiet-but-unique-nodes", help="Quiet mode. Except outputting number of unique nodes.",
                        action="store_true")
    group2 = parser.add_mutually_exclusive_group(required=True)
    group2.add_argument("-l", "--line-input", type=str,
                        help="Represent the input set in one line. Format: a|b|c&d|e|f ...")
    group2.add_argument("-lf", "--line-input-file", type=argparse.FileType('r'),
                        help="Represent the input set in one line stored in a file. Format: a|b|c&d|e|f ...")
    group2.add_argument("-d", "--dimacs", type=argparse.FileType('r'),
                        help="File name to contain the set in DIMACS format. See http://bit.ly/dimcasf")
    ...
    parser.add_argument("-ns", "--no-stats",
                        help="Short concise output. No stats about unique numbers and other information. This will disable the global database option.",
                        action="store_true")
    parser.add_argument("-t", "--threads", type=int,
                        help="Number of threads. Value 1 means no multithreading, 0 means max concurrent threads on the machine. This option will implicitly enable the global DB.",
                        default=1)
    ...
    parser.add_argument("-rdb", "--use-runtime-db",
                        help="Use database for set lookup in table established only for the current cnf",
                        action="store_true")
    parser.add_argument("-gdb", "--use-global-db", help="Use database for set lookup in global sets table",
                        action="store_true")
    parser.add_argument("-gnm", "--gdb-no-mem",
                        help="Don't load hashes from global DB into memory. Use this only if gdb gets huge and doesn't fit in memory. (slower)",
                        action="store_true")
    parser.add_argument("-z", "--sort-by-size", help="Always sort clauses by size in ascending order.",
                        action="store_true")
    parser.add_argument("-thief", "--thief-method", help="Always sort clauses by length and initial index.",
                        action="store_true")
    ...
    parser.add_argument('--version', action='version',
                        version='%(prog)s ')  # can use GitPython to automatically get latest tag here
    ...
    if args.line_input is None and args.line_input_file is None and args.dimacs is None:
        print("No input provided. Please provide any of the input arguments.")
        parser.print_help()
        sys.exit(3)

    # only one input must be provided
    if (args.line_input and args.line_input_file) or (args.line_input and args.dimacs) or (
            args.dimacs and args.line_input_file):
